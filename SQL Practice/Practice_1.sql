...
DELIMITER &&
create procedure avg_bal_jobrole2(IN sudh varchar(30))
BEGIN
	select  avg(balance) from bank_details where job = sudh;
END &&

call avg_bal_jobrole2('admin.');

call avg_bal_jobrole2('retired');

call avg_bal_jobrole2('unknown');
...
BEGIN
	select * from bank_details where eduction = v1 and job =v2;
END &&
